...
class CadastroUController {
    create(req, res){
        ...
        CadastroUsuario.inserir(nome_usuario,cpf_usuario,cep_usuario,estado_usuario,cidade_usuario,bairro_usuario,rua_usuario,telefone_usuario,email_usuario,nomeUser_usuario,senha_usuario)
            .then(resposta => {
    ...
    logar(req, res) {
        ...
        CadastroUsuario.verificaUsuarioSenha(email, senha)
            .then(resposta => {
                ...
                let token = '';
                if (resposta[0] === 200) {
                    token = jwt.sign({ usuario_id, usuario_tipo, nome_user_usuario }, secret, { expiresIn: 300 });
                }
